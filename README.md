...
| `showError`                 |  boolean         |   `false`    | Отображение информации о наличии ошибок во входящих значениях |
| `checkingStepSizeForScale`  |  boolean         |   `false`    | Проверка и установка шагу шкалы слайдера оптимального значения (заключается в проверке на то, что длина шкалы слайдера делится на равные части без остатка) |
